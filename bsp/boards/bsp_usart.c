...
void usart7_tx_dma_init(void)
{
    ...
    hdma_uart7_tx.Instance->PAR = (uint32_t) & (USART1->DR);
    hdma_uart7_tx.Instance->M0AR = (uint32_t)(NULL);
    hdma_uart7_tx.Instance->NDTR = 0;
...
void uart7_tx_dma_enable(uint8_t *data, uint16_t len)
{
    ...
    __HAL_DMA_CLEAR_FLAG(&hdma_uart7_tx, DMA_HISR_TCIF7);
