...
//机器人间自定义通信数据长度,小于等于113字节
#define STUDENT_DATA_SIZE   30

//自定义控制器交互数据长度,需小于等于30字节
...
typedef struct                  			//0x0001     11     比赛状态数据，3Hz 周期发送
{
    uint8_t game_type : 4;
...
typedef struct 								//0x0003    28   比赛机器人血量数据，3Hz 周期发送
{
...
typedef struct							//0x0004 	3		飞镖发射状态，飞镖发射后发送
{
	uint16_t v1;
    uint8_t v2;
}ext_dart_shoot_status_t;
...
typedef  struct  //0x0101   4     场地事件数据，3Hz 周期发送
{
    uint32_t event_type;
} ext_event_data_t;
...
typedef struct  //0x0105     1  飞镖发射口倒计时，3Hz 周期发送
{
...
typedef struct  //0x0204    1     机器人增益数据，1Hz发送
{ 
...
typedef struct  //0x0208     2    子弹剩余发送数，空中机器人以及哨兵机器人发送，10Hz 周期发送
{
...
typedef struct  //0x0209    4   机器人 RFID 状态，3Hz 周期发送
{
...
typedef struct       //裁判系统整个帧头
{
    uint8_t SOF;          // SOF码0xA5
    uint16_t Data_Length; //数据包长度 / 字节
    uint8_t Seq;          //包序号
    uint8_t CRC8;         // CRC8校验码
    uint16_t cmd_id;      //CMD_ID 0x301
}referee_frame_head;

typedef struct          //裁判系统数据头
{
    uint16_t data_id;   //学生自定义0x0200-0x02FF
    uint16_t sender_id;     //发送机器人的id
    uint16_t receiver_id;   //接收机器人的id
}referee_data_head;

typedef struct              //0x301 STUDENT_DATA_SIZE 机器人自定义数据
{
    referee_data_head head; //数据头

    __packed union 
    {
        uint8_t data[STUDENT_DATA_SIZE];        //数据存放区域
        
    }message;

}ext_student_data_t;

typedef __packed struct     //0x302 30Hz 自定义控制器下发数据
{
    __packed union
    {
        uint8_t data[CUSTOM_DATA_SIZE];     //缓冲区

        /**
         * @brief 自定义字段
         * 
         */
        //typdef struct {xxx}xxx;
    }controller_data;
}robot_interactive_data_t;

typedef struct              //小地图下发信息标识：0x0303。发送频率：触发时发送。
{
...
typedef struct              //图传遥控信息标识：0x0304。发送频率：30Hz。
{
int16_t mouse_x;            //鼠标x轴
int16_t mouse_y;            //鼠标z轴
int16_t mouse_z;            //鼠标滚轮
int8_t left_button_down;      //鼠标左键
int8_t right_button_down;     //鼠标右键
uint16_t keyboard_value;    //键盘按下情况， 0-15位各代表一个按键， 具体映射查阅RoboMaster2023裁判系统串口协议附录V1.4
uint16_t reserved;          //保留位
} ext_robot_keyborad_mouse_command_t;

//雷达机器人发送给客户端的， 本模块暂不支持该功能
typedef struct                  //客户端接收信息标识：0x0305。最大接收频率：10Hz。
{
...
extern ext_student_data_t receive_robot_data;                                 //其他机器人发送过来的信息
extern ext_student_data_t send_robot_data;                                    //向其他机器人发送的信息
extern robot_interactive_data_t custom_data;                                  //自定义控制器信息
extern ext_robot_command_t ext_robot_command;                                 //小地图下发信息标识
extern ext_robot_keyborad_mouse_command_t ext_robot_keyborad_mouse_command;
/**
...
/**
 * @brief 发送自定义机器人交互信息 10Hz
 * 
 * @details 注意， 该函数和UI_Refresh函数一样， 并不是真正的通过串口把数据发送出去， 而是把数据先保存至缓冲区，
 * 使用send(queue*)函数进行发送， 所以， 如果有ui任务可以把ui任务和机器人交互数据任务一起做， 并10Hz调用send(&queue);
 * @param data              具体数据
 * @param data_id           自定义数据ID 0x0200-0x02FF
 * @param target_robot_id   目标机器人ID 
 */
void referee_send(ext_student_data_t* data, uint16_t data_id, uint16_t receiver_id);
