...
static void gimbal_init(gimbal *gimbal_init);
void gimbal_loop(gimbal *loop);
static void gimbal_feedback_update(gimbal *feedback_update);
static void gimbal_rc_to_movement(gimbal *movement);
...
void gimbal_task(void const *pvParameters)
{
	...
	for(;;)
	{
//		 gimbal_rc_to_movement(&gimbal_ctrl);
...
static void gimbal_rc_to_movement(gimbal *movement)   //遥控器控制云台角度
{
...
//	gimbal_init->gimbal_rc = &rc_ctrl; //获取遥控器值
	...
	gimbal_init->gimbal_pitch_motor.motor_gyro_set = gimbal_init->gimbal_pitch_motor.motor_gyro_set ;//陀螺仪初始化
...
//fp32 quadratic_function(fp32 rate_min,fp32 rate_max,)
...
void gimbal_loop(gimbal *loop)
{
	...
	if(loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE	) //自稳模式
	{
		 gimbal_rc_to_movement(&gimbal_ctrl);      //只计算云台遥感设置值
   ...
   static int16_t yaw_channel = 0, pitch_channel = 0;
    ...
    //Visual_Ture.flag   判断是否收到数据
		if((Visual_Ture.flag==1)&&( Visual_Ture.RInit_data[0] == 0x73) && (Visual_Ture.RInit_data[7] == 0x65 ))
		{
		  ...
		  Visual_Ture.RDecode_data[1] = (Visual_Ture.RInit_data[3] | Visual_Ture.RInit_data[4] << 8) ; //y
			pitch_motor_visual_date=Visual_Ture.RDecode_data[1];//原始数据观测值
			
		Visual_Ture.Coordinates[0] = Visual_Ture.RDecode_data[0] ;
		Visual_Ture.Coordinates[1] = Visual_Ture.RDecode_data[1] ;
			
		loop->gimbal_yaw_motor.visual_pixel = KalmanFilter(&visual_x_fillter,(Visual_Ture.RDecode_data[0])*1.0); //这个符号匹配Yaw和pitch的方向
		loop->gimbal_pitch_motor.visual_pixel = KalmanFilter(&visual_y_fillter,(Visual_Ture.RDecode_data[1])*1.0);

			
		yaw_motor_visual_pixel=loop->gimbal_yaw_motor.visual_pixel;
		pitch_motor_visual_pixel=loop->gimbal_pitch_motor.visual_pixel;
    ...
    rc_deadband_limit(loop->gimbal_rc->rc.ch[gimbal_pitch_CHANNEL], pitch_channel, RC_DEADBAND);
		...
		if(fabs(loop->gimbal_yaw_motor.visual_pixel)<150)
		{
			rate_yaw=2;
		...
		if(fabs(loop->gimbal_pitch_motor.visual_pixel)<150)
		{
			rate_pitch=2;
    ...
    loop->add_yaw = loop->gimbal_yaw_motor.visual_pixel * Motor_Angle_to_Rad_YAW + yaw_channel * YAW_RC_SEN - loop->gimbal_rc->mouse.x * YAW_MOUSE_SEN;
    loop->add_pitch = loop->gimbal_pitch_motor.visual_pixel * Motor_Angle_to_Rad_PITCH+ pitch_channel * PITCH_RC_SEN + loop->gimbal_rc->mouse.y * PITCH_MOUSE_SEN;

    loop->gimbal_yaw_motor.absolute_angle_set = loop->gimbal_yaw_motor.absolute_angle_set + loop->add_yaw;   //计算yaw轴目标角度值
    gimbal_absolute_angle_limit(&loop->gimbal_pitch_motor, loop->add_pitch);
		if(fabs(loop->gimbal_pitch_motor.absolute_angle_set-loop->gimbal_pitch_motor.absolute_angle)>0.4)
		{
	  ...
	  gimbal_rc_to_movement(&gimbal_ctrl);   //计算absolute_angle_set值
    ...
    loop->gimbal_pitch_motor.current_set = PID_calc(&loop->gimbal_pitch_motor.gimbal_motor_gyro_pid, 
	  ...
	  if(loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_ZERO)//如果云台关闭则清除所有输出值
		...
		CAN_cmd_gimbal( -loop->gimbal_yaw_motor.current_set/**YAW_DIRECTION*/, loop->gimbal_pitch_motor.current_set, 0, 0);
