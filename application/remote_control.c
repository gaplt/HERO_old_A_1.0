...
void remote_control_init(void)
{
    RC_Init(sbus_rx_buf[0], sbus_rx_buf[1], SBUS_RX_BUF_NUM);
		...
		kalmanCreate(&mouse_x_fillter, 14, 400);
		kalmanCreate(&mouse_y_fillter, 14, 400);
		kalmanCreate(&mouse_z_fillter, 14, 400);
}
/**
...
static void sbus_to_rc(volatile const uint8_t *sbus_buf, RC_ctrl_t *rc_ctrl)
{
    ...
    rc_ctrl->rc.ch[4] -= RC_CH_VALUE_OFFSET;
		...
		rc_ctrl->mouse.x = KalmanFilter(&mouse_x_fillter, rc_ctrl->mouse.x);
		rc_ctrl->mouse.y = KalmanFilter(&mouse_y_fillter, rc_ctrl->mouse.y);
		rc_ctrl->mouse.z = KalmanFilter(&mouse_z_fillter, rc_ctrl->mouse.z);
}
